...
class Appel{
	...
	public function startMenu(){
		...
		while(!$done){
			$keys = null;
			$ret = $this->agi->text2wav('for sales press 1, for billing press 2, for support press 3', '#123');
			if($ret['result'] != 0){
				$keys = chr($ret['result']);
				$done = true;
			}
			...
			switch ($keys){
				case '1':
					$this->agi->text2wav('Welcom to sales');
					$done=true;
					break;
				case '2':
					$this->agi->text2wav('Welcom to billing');
					$done=true;
					break;
				case '3':
					$this->agi->text2wav('Welcom to support');
					$done=true;
					break;
				default:
					$this->agi->text2wav('Do not get right choice, please try again');
					break;
			}
		}
		return $keys;
	 ...
	 */
	public function getDestination($message, $num = 15){
		$done=false;
		while(!$done){
			$keys = null;
			$ret = $this->agi->text2wav($message,'1234567890');
			if($ret['result'] != 0){
				$keys = chr($ret['result']);
				$this->agi->conlog($keys);
				$done = true;
